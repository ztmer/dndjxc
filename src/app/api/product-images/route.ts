...
import { saveProductJpeg } from "@/lib/product-image-store";
...
export async function POST(req: Request) {
  ...
  if (file.size > MAX) return NextResponse.json({ error: "图片太大，请裁小后再传" }, { status: 400 });
  ...
  if (!jpeg) return NextResponse.json({ error: "请先在修图里点确定（会存成照片）" }, { status: 400 });
  const saved = await saveProductJpeg(buf);
